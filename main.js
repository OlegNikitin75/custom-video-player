...
const sliderItems = document.querySelectorAll('.video-library__item');
...
const gap = (sliderWidth - (sliderItemWidth * 3)) / 2;
sliderlist.style.width = (sliderLength * sliderItemWidth) + (gap * sliderLength - 1) + 'px';
...
const showNextSlides = () => {
  counterSlides++;
  offset += sliderWidth + gap;
  if (counterSlides === srcs.length / 3) {
    offset = 0;
    counterSlides = 0;
  }
...
const showPrevSlides = () => {
  ...
  offset -= sliderWidth + gap;
  if (counterSlides < 0) {
    offset = (sliderLength * sliderItemWidth) + (gap * sliderLength - 1) - sliderWidth - gap;
    counterSlides = srcs.length / 3 - 1;
  }
  sliderlist.style.left = -offset + 'px';
}
//========================================
...
//==========================================
const turnSound = () => {
  let currentVolumeValue = stack.pop() * 100;
  volumeControl.value = currentVolumeValue;
...
const toggleFullScreen = () => {
  if (!document.fullscreenElement) {
    ...
    library.classList.add('fullscreen-library');
  } else {
    if (document.exitFullscreen) {
      ...
      library.classList.remove('fullscreen-library');
    }
  }
}
//+++++++++++++++++++++++++++++++++++++++++++
